...
export const FringeSearch = async function(){
   let fringe = [this.state.start];
    start=this.state.start;
    heuristics = this.state.heuristics;
    grid = this.state.grid;
    end = this.state.end[0];//does the work of both lists 'new' and 'later' by storing fringes
    ...
    const cache = Array(this.state.height).fill(undefined, undefined, undefined).map(() => Array(this.state.width).fill(0));
    ...
    let flimit = heuristics[start[0][0]][start[0][1]]; //initial threshold
    let found = false;
